...
export const useObservations = (transcript, patientId) => {
  ...
  const fetchHistoricalData = useCallback(async () => {
    ...
    try {
      ...
      const questionnaireResponseUrl = `${FHIR_SERVER}/QuestionnaireResponse?patient=${patientId}&_count=100`;

      const [fetchedObservations, fetchedQuestionnaireResponses] = await Promise.all([
        fetchAllPages(observationUrl),
        fetchAllPages(questionnaireResponseUrl)
    ]);

    console.log("Fetched observations:", fetchedObservations);
    console.log("Fetched questionnaire responses:", fetchedQuestionnaireResponses);

    const processedObservations = observationsRef.current.map(obs => {
      let updatedHistory = [...obs.history];
      let updatedValue = obs.value;

      const matchingResources = (obs.fhirResource === 'Observation' ? fetchedObservations : fetchedQuestionnaireResponses).filter(o => {
        const resource = o.resource;
        const codeText = resource.code?.text || '';
        const codingDisplays = resource.code?.coding?.map(c => c.display) || [];

        return codeText === obs.label || codingDisplays.includes(obs.label) || resource.item?.some(item => item.linkId === obs.label);
      });

      if (matchingResources.length > 0) {
        matchingResources.forEach(o => {
          const resource = o.resource;
          let value = '';
          ...
          if (resource.resourceType === 'Observation') {
            ...
            resource.item?.forEach(item => {
              if (item.linkId === obs.label) {
                item.answer?.forEach(answer => {
                  value = answer.valueString || answer.valueQuantity?.value?.toString() || '';
                });
              }
            });
          }
  ...
  return { observations, setObservations, isLoading, fetchObservations };
};